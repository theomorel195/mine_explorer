...
def test_xacro_parsing():
    ...
    pkg_name = 'arm_mine_explorer_description'
    try:
    ...
    assert 'world' in robot_xml, 'The root link world was not found in URDF.'


def test_config_files_existence():
    """Verify that all required configuration YAML files exist in the config folder."""
    ...
    for config in required_configs:
        config_path = os.path.join(pkg_path, 'config', config)
        assert os.path.exists(config_path), f'Missing configuration file: {config_path}'
